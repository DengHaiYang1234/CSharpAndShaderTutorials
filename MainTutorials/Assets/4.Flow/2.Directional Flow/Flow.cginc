...
float3 FlowUVW(float2 uv,float2 flowVector,float2 jump,float flowOffest,float tiling,float time,bool flowB)
{
	//flowB的起始UV坐标为0.5
	...
	//跳跃效果只需将flowVector设置为0就可以很清楚的看见
...
float2 DirectionFlowUV(float2 uv,float3 flowVectorAndSpeed,float tiling,float time,out float2x2 rotation)
{
